...
const ShakedGame = () => {
    ...
    const start = () => {
        const _toys = allItems.map(x => ({ x, r: Math.random() }))
            .sort((a, b) => a.r - b.r)
            .map(a => a.x)
            .slice(0, TOYS_COUNT(level+1));

        const _items = allItems.map((item) => {
            const left = (Math.random() * (BOARD_SIZE(level+1) - ITEM_SIZE(level+1))).toFixed();
            const top = (Math.random() * (BOARD_SIZE(level+1) - ITEM_SIZE(level+1))).toFixed();
